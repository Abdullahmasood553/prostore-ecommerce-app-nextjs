...
// Approve paypal order and approve order to paid
export async function approvePayPalOrder(  orderId: string,
  ...
  try {
    ...
    if (!captureData || captureData.data !== (order.paymentResult as PaymentResult)?.id || captureData.status !== 'COMPLETED') {
...
async function updateOrderToPaid({orderId, paymentResult} : {orderId: string, paymentResult? : PaymentResult}) {
  ...
  // Transaction to update order and account for paid stock
